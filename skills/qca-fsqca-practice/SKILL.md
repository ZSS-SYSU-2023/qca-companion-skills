...
### 2.11 本地执行与交叉验证（Python 执行层 / R / fsQCA 桌面版）

> 本包只做方法指引与校对，计算交给执行层与专业软件；三者并行时按下面分工取信。

**三层职责**
| 层 | 工具 | 负责 | 取信原则 |
|---|---|---|---|
| 执行层 | 本地技能 `qca-analysis-expert`（Python：numpy/pandas，自建 QCA 计算与项目工程结构 `.tasks/data/results/logs`） | 端到端跑流程、产出图表与日志 | 流程与工程化听它的 |
| 算法基准 | R `QCA` 包（CCubes 算法，数学精确的最小化） | 真值表、最小化、必要性/充分性 | **最小化结果以它为准** |
| GUI 复核 | fs/QCA 桌面版（Qt 图形界面） | 快速复核、教学演示、与合著者对齐 | 与 R 结果应一致 |

- **为什么要交叉验证**：执行层多为自实现的近似求解，条件数多、逻辑余项多时可能与精确算法不一致。用仓库内 `examples/r_qca_minimal_demo.R`（csQCA + fsQCA 双轨）跑同一份数据，与执行层结果对照，一致再进入写作。
- **执行层的方法论红线**见本地 `qca-analysis-expert` 技能"六大绝对禁止原则"（禁止脱离理论校准、禁止忽略案例独特性等），本包各闸门与其互补：它管分析纪律，本包管方法与写作规范。
- **真实数据练习**：用《管理世界》公开论文的复现数据（CSV，含预校准模糊集列）走一遍全流程，比合成数据更能暴露校准与阈值的取舍问题。

**常见报错排查（R / QCA 3.x 实测）**
| 报错 | 原因 | 处理 |
|---|---|---|
| `Threshold value(s) not specified` | fuzzy `calibrate()` 未给阈值 | 显式给 3 个（完全隶属/交叉点/完全不隶属）或 6 个阈值，可用 95/50/5 分位 |
| `there should be either 3 or 6 thresholds` | 只给了 2 个阈值（如仅首尾分位） | 补上交叉点，通常取中位数 |
| `Missing values in the data are not allowed` | 数据含 NA，或把 0/1 已校准数据再拿去 fuzzy 校准 | 先处理缺失（剔除/插补）；已是 0/1 清晰集的数据不要重复校准 |
| 中文列乱码 | CSV 多为 GBK/GB2312 编码 | `read.csv(..., fileEncoding = "GBK")` |
| 中间解出现十几/二十几条 | 条件数偏多、逻辑余项过多 | 减少条件数（合并/理论取舍）、提高频数阈值，或改用中间解+方向性预期约束 |
